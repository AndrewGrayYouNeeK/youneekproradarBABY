...
This app uses [Apple WeatherKit](https://developer.apple.com/weatherkit/) for current conditions and forecasts via `/api/weather`. Radar and NWS alerts work without Apple credentials; WeatherKit is optional but higher quality for forecast data.
...
## Step 5 — Production (Cloudflare Pages)

Set secrets on the `youneek-pro-radarynk222` Pages project (never commit these):

```bash
npx wrangler pages secret put WEATHERKIT_TEAM_ID --project-name=youneek-pro-radarynk222
npx wrangler pages secret put WEATHERKIT_KEY_ID --project-name=youneek-pro-radarynk222
npx wrangler pages secret put WEATHERKIT_SERVICE_ID --project-name=youneek-pro-radarynk222
npx wrangler pages secret put WEATHERKIT_PRIVATE_KEY --project-name=youneek-pro-radarynk222
```
...
| `lat` | Yes | Latitude |
| `lon` | Yes | Longitude |
| `dataSets` | No | Comma-separated WeatherKit data sets. Default: `currentWeather,forecastHourly,forecastDaily,forecastNextHour,weatherAlerts`. The function always sends `country=US` so Apple weather alerts can return. |
...
| Works locally but not in production | Confirm all four secrets exist on the Pages project |
